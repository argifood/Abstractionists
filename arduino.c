...
void getPumps(){
  ...
  String response = "";
  //canLoop = false;
/*  Serial.write("AT+HTTPPARA=\"URL\",\"abstractionists.alwaysdata.net/api/getPumps.php\"");
  delay(2000);
  ...
  response = callCommand("AT+HTTPPARA=\"URL\",\"abstractionists.alwaysdata.net/api/getPumps.php\"");
  if(response.equals("ERROR")){
    Serial.println("Something went wrong, restart");
    return;
  }
  else{
    response = callHttpCommand("AT+HTTPACTION=0");
    if(response.equals("ERROR")){
      Serial.println("Something went wrong, restart");
      return;
    }
      ...
      response = "";
      response = callCommand("AT+HTTPREAD");
      if(response.equals("ERROR")){
        Serial.println("Something went wrong, restart");
        return;
      }
